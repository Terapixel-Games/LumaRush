...
Current state:
- [x] Unit verification is clean via `verify_repo.ps1 -Suite unit` (files=25 tests=98 failures=0).
- [x] 180-frame visual smoke is clean via `rush_combo_push`.
...
- [ ] Gate or mock Nakama client-event publishing during offline visual-smoke runs to remove expected `http error` warnings.
