...
func get_invariants() -> Array[Dictionary]:
	return [
		{"id": "actions_non_idle", "metric": "actions_total", "op": ">=", "value": 18},
		{"id": "rerun_transition", "metric": "runs_started", "op": ">=", "value": 2},
		{"id": "score_progress", "metric": "score_final", "op": ">=", "value": 80},
	]
...
func _apply_remove_color(game_scene: Node) -> bool:
	var board_view: Node = game_scene.get_node_or_null("BoardView")
	if board_view == null:
		return false
	var board = board_view.get("board")
	if board == null:
		return false
	var color_idx: int = int(board_view.call("_best_removal_color"))
	if color_idx < 0:
		return false
	var removed: int = int(board.remove_color(color_idx, int(board_view.call("_palette_size"))))
	if removed <= 0:
		return false
	board_view.call("_normalize_board_color_ids")
	board_view.call("_rebuild_tiles_from_grid")
