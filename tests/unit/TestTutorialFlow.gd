...
func test_first_powerup_use_prompts_for_open_leaderboard_and_tutorial_resets_prompt() -> void:
	...
	assert_that(panel.size.y).is_greater_equal(276.0)
	...
	assert_that(message_rect.position.y - title_rect.end.y).is_greater_equal(12.0)
	...
	assert_that(panel_rect.end.y - checkbox_rect.end.y).is_greater_equal(18.0)
	assert_that(panel_rect.end.y - confirm_rect.end.y).is_greater_equal(18.0)
